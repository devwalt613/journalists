...
DEFAULT_CHANNELS = (
    "GbmMDm,taagad_news,Moshepargod,offtherecord1,arutz1newss"
)

CHANNELS = [c.strip() for c in os.environ.get("TELEGRAM_CHANNELS", DEFAULT_CHANNELS).split(",") if c.strip()]

# Each of these channels ends every real post with a promo/footer block
# (WhatsApp/Telegram links, "send us tips" numbers, etc). Ads/promos posted
# on their own don't contain this marker text at all, so we use its
# presence/absence to both strip the footer AND decide whether to skip the
# post entirely. If you find a channel's footer wording changed and posts
# stop getting sent (or the footer stops getting stripped), update the
# matching entry below.
CHANNEL_CUTOFFS = {
    "GbmMDm": "לשליחת חומרים ולפרסום",
    "taagad_news": "להצטרפות ל'זמן אמת'",
    "Moshepargod": "לעדכוני הפרגוד",
    "offtherecord1": "לשליחת חומרים:",
    "arutz1newss": "ערוץ 1 - תוכן בלעדי בלבד",
}
...
def strip_footer(channel, text):
    """
    Cut off the promo/footer block these channels append to every real
    post (WhatsApp/Telegram links, tip lines, etc), using a per-channel
    marker string. If the marker isn't found at all, the post is treated
    as a standalone ad/promo (not a real post) and is skipped entirely —
    return None in that case.
    """
    cutoff = CHANNEL_CUTOFFS.get(channel)
    if not cutoff:
        return text  # no marker configured for this channel, leave as-is

    idx = text.find(cutoff)
    if idx == -1:
        return None  # no footer marker present at all -> treat as ad, skip

    trimmed = text[:idx].rstrip()
    if not trimmed:
        return None  # nothing left above the footer -> treat as ad, skip
...
def fetch_messages(channel):
    ...
    for block in soup.select("div.tgme_widget_message"):
        ...
        # Strip this channel's promo footer, or skip the post entirely if
        # it turns out to be a standalone ad/promo with no footer marker.
        if text:
            text = strip_footer(channel, text)
        if not text:
            continue
